import { Omit } from "lodash";

export class ValidatorRules {
    private constructor(private readonly value: any, private readonly property: string) {}


    static values(value: any, property: string): ValidatorRules {
        return new ValidatorRules(value, property);
    }

    required(): Omit<this, 'required'> {
        if(this.value === null || this.value === undefined || this.value === ''){
            throw new Error(`${this.property} is required`);
        }
        
        return this;
    }

    string(): Omit<this, 'string'> {
        if(!isEmpty(this.value) && typeof this.value !== 'string'){
            throw new Error(`${this.property} must be a string`);
        }
        return this
    }

    boolean(): Omit<this, 'boolean'> {
        if(!isEmpty(this.value) && typeof this.value !== 'boolean'){
            throw new Error(`${this.property} must be a boolean`);
        }
        return this
    }

    maxLength(max: number): Omit<this, 'maxLength'> {
        if(!isEmpty(this.value) && this.value.length > max){
            throw new Error(`${this.property} must be less than ${max} characters`);
        }
        return this;
    }

}

export function isEmpty(value: any): boolean {
    return value === null || value === undefined || value === '';
}

export default ValidatorRules;